...
export function renderHistoryPanel() {
    ...
    history.forEach(entry => {
        ...
        titleSpan.className = 'font-medium text-sm pointer-events-none text-text-primary';
        titleSpan.textContent = entry.title;
        titleSpan.title = entry.title;
        
        const dateSpan = document.createElement('span');
        dateSpan.className = 'block text-xs text-text-muted mt-1 pointer-events-none';
        ...
        deleteButton.className = 'p-2 rounded-full hover:bg-destructive-subtle-bg text-destructive/70 hover:text-destructive transition-colors';
