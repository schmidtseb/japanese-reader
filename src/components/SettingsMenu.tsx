...
export function SettingsMenu({ setIsOpen }: { setIsOpen: (isOpen: boolean) => void }) {
    ...
    const handleImport = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                showAlert(`Import failed: ${err instanceof Error ? err.message : 'Could not read file.'}`);
            } finally {
                event.target.value = '';
            }
        };
    ...
    const handleExport = () => {
        const backupData = {
            version: '2.0-react',
            exportedAt: Date.now(),
            history: appDataState.history,
            reviewDeck: appDataState.reviewDeck,
    ...
    return (
        <div ref={menuRef} className="absolute top-full right-0 mt-3 w-[95vw] max-w-sm glass-morphism rounded-xl shadow-2xl ring-1 ring-border-subtle z-20 origin-top-right animate-fade-in">
